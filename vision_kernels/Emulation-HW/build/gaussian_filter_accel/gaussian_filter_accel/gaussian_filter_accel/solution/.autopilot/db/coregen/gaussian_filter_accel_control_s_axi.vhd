...
entity gaussian_filter_accel_control_s_axi is
generic (
...
end entity gaussian_filter_accel_control_s_axi;
...
architecture behave of gaussian_filter_accel_control_s_axi is
    ...
    constant ADDR_SIGMA_DATA_0   : INTEGER := 16#38#;
    constant ADDR_SIGMA_CTRL     : INTEGER := 16#3c#;
    ...
    signal int_sigma           : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_SIGMA_DATA_0 =>
                        rdata_data <= RESIZE(int_sigma(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    sigma                <= STD_LOGIC_VECTOR(int_sigma);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SIGMA_DATA_0) then
                    int_sigma(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_sigma(31 downto 0));
                end if;
            end if;
        end if;
    end process;
