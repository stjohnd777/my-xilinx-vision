...
// ==============================================================
`timescale 1ns/1ps
module remap_accel_control_s_axi
#(parameter
    ...
    output wire [63:0]                   map_x,
    output wire [63:0]                   map_y,
...
localparam
    ...
    ADDR_IER            = 7'h08,
    ADDR_ISR            = 7'h0c,
    ADDR_IMG_IN_DATA_0  = 7'h10,
    ADDR_IMG_IN_DATA_1  = 7'h14,
    ADDR_IMG_IN_CTRL    = 7'h18,
    ADDR_MAP_X_DATA_0   = 7'h1c,
    ADDR_MAP_X_DATA_1   = 7'h20,
    ADDR_MAP_X_CTRL     = 7'h24,
    ADDR_MAP_Y_DATA_0   = 7'h28,
    ADDR_MAP_Y_DATA_1   = 7'h2c,
    ADDR_MAP_Y_CTRL     = 7'h30,
    ADDR_IMG_OUT_DATA_0 = 7'h34,
    ADDR_IMG_OUT_DATA_1 = 7'h38,
    ADDR_IMG_OUT_CTRL   = 7'h3c,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_map_x = 'b0;
    reg  [63:0]                   int_map_y = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_MAP_X_DATA_0: begin
                    rdata <= int_map_x[31:0];
                end
                ADDR_MAP_X_DATA_1: begin
                    rdata <= int_map_x[63:32];
                end
                ADDR_MAP_Y_DATA_0: begin
                    rdata <= int_map_y[31:0];
                end
                ADDR_MAP_Y_DATA_1: begin
                    rdata <= int_map_y[63:32];
                end
...
assign map_x         = int_map_x;
assign map_y         = int_map_y;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_MAP_X_DATA_0)
            int_map_x[31:0] <= (WDATA[31:0] & wmask) | (int_map_x[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_MAP_X_DATA_1)
            int_map_x[63:32] <= (WDATA[31:0] & wmask) | (int_map_x[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_MAP_Y_DATA_0)
            int_map_y[31:0] <= (WDATA[31:0] & wmask) | (int_map_y[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_MAP_Y_DATA_1)
            int_map_y[63:32] <= (WDATA[31:0] & wmask) | (int_map_y[63:32] & ~wmask);
    end
end
