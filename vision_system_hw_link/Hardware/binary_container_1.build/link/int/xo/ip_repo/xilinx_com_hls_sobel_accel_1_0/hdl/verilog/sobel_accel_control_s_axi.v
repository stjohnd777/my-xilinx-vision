...
// ==============================================================
`timescale 1ns/1ps
module sobel_accel_control_s_axi
#(parameter
    ...
    output wire [63:0]                   img_out1,
    output wire [63:0]                   img_out2,
...
localparam
    ...
    ADDR_IMG_OUT1_DATA_0 = 7'h1c,
    ADDR_IMG_OUT1_DATA_1 = 7'h20,
    ADDR_IMG_OUT1_CTRL   = 7'h24,
    ADDR_IMG_OUT2_DATA_0 = 7'h28,
    ADDR_IMG_OUT2_DATA_1 = 7'h2c,
    ADDR_IMG_OUT2_CTRL   = 7'h30,
    ADDR_ROWS_DATA_0     = 7'h34,
    ADDR_ROWS_CTRL       = 7'h38,
    ADDR_COLS_DATA_0     = 7'h3c,
    ADDR_COLS_CTRL       = 7'h40,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
    WRRESP               = 2'd2,
    WRRESET              = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_img_out1 = 'b0;
    reg  [63:0]                   int_img_out2 = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_IMG_OUT1_DATA_0: begin
                    rdata <= int_img_out1[31:0];
                end
                ADDR_IMG_OUT1_DATA_1: begin
                    rdata <= int_img_out1[63:32];
                end
                ADDR_IMG_OUT2_DATA_0: begin
                    rdata <= int_img_out2[31:0];
                end
                ADDR_IMG_OUT2_DATA_1: begin
                    rdata <= int_img_out2[63:32];
                end
...
assign img_out1      = int_img_out1;
assign img_out2      = int_img_out2;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IMG_OUT1_DATA_0)
            int_img_out1[31:0] <= (WDATA[31:0] & wmask) | (int_img_out1[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_img_out1[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_IMG_OUT1_DATA_1)
            int_img_out1[63:32] <= (WDATA[31:0] & wmask) | (int_img_out1[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_IMG_OUT2_DATA_0)
            int_img_out2[31:0] <= (WDATA[31:0] & wmask) | (int_img_out2[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_img_out2[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_IMG_OUT2_DATA_1)
            int_img_out2[63:32] <= (WDATA[31:0] & wmask) | (int_img_out2[63:32] & ~wmask);
    end
end
