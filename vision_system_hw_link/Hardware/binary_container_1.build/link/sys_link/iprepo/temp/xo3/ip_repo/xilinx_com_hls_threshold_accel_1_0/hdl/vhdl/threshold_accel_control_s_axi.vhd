...
entity threshold_accel_control_s_axi is
generic (
...
end entity threshold_accel_control_s_axi;
...
-- 0x24 : reserved
-- 0x28 : Data signal of thresh
--        bit 7~0 - thresh[7:0] (Read/Write)
--        others  - reserved
-- 0x2c : reserved
-- 0x30 : Data signal of maxval
--        bit 7~0 - maxval[7:0] (Read/Write)
--        others  - reserved
-- 0x34 : reserved
-- 0x38 : Data signal of rows
--        bit 31~0 - rows[31:0] (Read/Write)
-- 0x3c : reserved
-- 0x40 : Data signal of cols
--        bit 31~0 - cols[31:0] (Read/Write)
-- 0x44 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of threshold_accel_control_s_axi is
    ...
    constant ADDR_THRESH_DATA_0  : INTEGER := 16#28#;
    constant ADDR_THRESH_CTRL    : INTEGER := 16#2c#;
    constant ADDR_MAXVAL_DATA_0  : INTEGER := 16#30#;
    constant ADDR_MAXVAL_CTRL    : INTEGER := 16#34#;
    constant ADDR_ROWS_DATA_0    : INTEGER := 16#38#;
    constant ADDR_ROWS_CTRL      : INTEGER := 16#3c#;
    constant ADDR_COLS_DATA_0    : INTEGER := 16#40#;
    constant ADDR_COLS_CTRL      : INTEGER := 16#44#;
    ...
    signal int_thresh          : UNSIGNED(7 downto 0) := (others => '0');
    signal int_maxval          : UNSIGNED(7 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_THRESH_DATA_0 =>
                        rdata_data <= RESIZE(int_thresh(7 downto 0), 32);
                    when ADDR_MAXVAL_DATA_0 =>
                        rdata_data <= RESIZE(int_maxval(7 downto 0), 32);
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_THRESH_DATA_0) then
                    int_thresh(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_thresh(7 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_MAXVAL_DATA_0) then
                    int_maxval(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_maxval(7 downto 0));
                end if;
            end if;
        end if;
    end process;
